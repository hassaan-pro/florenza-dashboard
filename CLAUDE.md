...
src/
  app/
    ...
    api/
      ...
      hosting/
        ...
        deploy/route.ts          # Zips the static export + Netlify Function and deploys to Netlify via the user's own token
      checkout/
        create-session/route.ts  # Real Stripe Checkout session creation for the in-app preview's cart
  ...
  lib/
    ...
    export-html.ts               # Plain-string-template HTML generation for Website Builder's export/deploy (deliberately not React SSR — see Website Builder notes), including the vanilla-JS cart/checkout
    netlify-function.ts          # Source strings for the Netlify Function + netlify.toml bundled into every Hosting deploy, powers the deployed site's checkout
...
Section-based builder for the Florenza storefront: add/reorder/edit content blocks, live preview with a working cart and checkout, device toggle, real domain verification, real hosting deploy, and real payment processing. Read this section before extending it, the scope was deliberately drawn in a specific place and it's easy to accidentally promise more than the code delivers.
...
- **The cart and checkout are real**, not a mock. `site-preview.tsx` has an actual cart (add/remove/adjust quantity, running subtotal) and a "Checkout with Stripe" button that calls `src/app/api/checkout/create-session/route.ts`, which hits Stripe's REST API directly (no `stripe` SDK dependency, just `fetch` with the person's own secret key from the **Payments** dialog) and creates a real Checkout Session. The browser is redirected to Stripe's actual hosted payment page. Test it end-to-end right now with a Stripe test key (`sk_test_…`) and Stripe's test card `4242 4242 4242 4242`, no real account or real money needed to see the full flow work.
...
- **The deployed site's checkout is also real**, and works independently of this dashboard being open. Every Hosting deploy bundles a Netlify Function (`netlify/functions/create-checkout-session.js`, sourced from `src/lib/netlify-function.ts`) plus a `netlify.toml` declaring it. That function is zero-dependency (native `fetch`, no npm install step, since the deploy is a raw zip with no build phase) and creates a Stripe Checkout Session the same way the in-app preview does, except it reads its Stripe key from a `STRIPE_SECRET_KEY` environment variable set directly in that Netlify site's own settings, never from anything typed into this dashboard. The exported HTML's cart is vanilla JS with a `localStorage`-backed cart (see `cartScript()` in `export-html.ts`), it works with zero framework at runtime.
- **HTML generation is plain string templating, not React SSR.** `src/lib/export-html.ts` intentionally does *not* import `<SitePreview>` or `react-dom/server` — App Router route handlers run under React's `react-server` module condition, which blocks importing `react-dom/server` (Next/Turbopack errors on it at build time). So the exported HTML, including its cart/checkout markup and JS, is generated by hand-written template strings in `export-html.ts` that mirror `site-preview.tsx`'s structure and behavior. **These two files can drift out of sync** — if you change the cart, a block's layout, or checkout logic in `site-preview.tsx`, update the matching code in `export-html.ts` by hand (and `src/app/api/checkout/create-session/route.ts` in lockstep with `netlify-function.ts`'s Stripe request-building, they're two independent implementations of the same request). There's no shared source of truth between the in-app preview and the exported/deployed HTML.
...
- No inventory, order storage, shipping calculation, tax handling, or email receipts beyond whatever Stripe Checkout itself provides out of the box. Stripe Checkout collects payment and can email a receipt; everything past "payment succeeded" (fulfillment, order records, notifying Florenza someone bought something) needs a webhook handler and a database, neither of which exist here. Don't describe this as a complete order-management pipeline, it's a working payment collection flow, not a back office.
- The `index.html`-only download (no token, no Netlify) still has cart UI but its checkout button will 404 unless deployed somewhere that provides the matching function — see the note in `hosting-dialog.tsx`.
...
- `payments-dialog.tsx` — holds `stripeKey`/`currency` state (lifted to the page, passed into `SitePreview`), shows test-vs-live mode based on the key prefix, explains the split between this key (used only for the in-app preview checkout) and the deployed site's separate `STRIPE_SECRET_KEY` env var
...
- **Website Builder now has a real hosting path (Netlify, bring-your-own-token) and real payment collection (Stripe, bring-your-own-key), but it's not a full production pipeline.** What's still missing: real product photography (placeholders only), a compiled stylesheet instead of the Tailwind CDN script for the exported HTML, any automation connecting a verified domain to the deployed Netlify site (manual step in Netlify's dashboard/DNS), and — this is the big one — **no order management behind the payment**. Stripe Checkout collects money and can email a receipt; there's no webhook handler, no database, nothing that tells Florenza an order came in or what to fulfill. Payment succeeding and an order being fulfillable are two different problems, only the first one is solved. Read the Website Builder section above in full before touching this, it's the section most likely to get oversold if summarized from memory.
- The in-app checkout test (Payments dialog key) and the deployed site's checkout (Netlify env var) are two separate credentials by design, don't try to "simplify" this into one shared key without thinking through why they're split (see the Website Builder section).
...
- `recharts` (Product Management), `fast-xml-parser` (News Consolidator), `@radix-ui/react-dropdown-menu` (Website Builder's "Add block" menu, via `ui/dropdown-menu.tsx`), and `jszip` (Website Builder's Netlify zip-deploy) were added as dependencies for their specific sections. Reuse them rather than introducing alternatives. Note the Stripe integration deliberately has **no** `stripe` SDK dependency, both the in-app route and the deployed Netlify Function call Stripe's REST API directly via `fetch` — keep it that way, the Netlify Function specifically can't `npm install` since deploys are raw zips with no build step.
